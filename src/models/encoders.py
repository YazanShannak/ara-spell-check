...
class EmbeddingEncoder(BaseEncoder):
    def __init__(self, vocab_count: int, latent_dim: int, pad_index: int, embedding_dim: int, dropout_ratio: float = 0.5, num_layers: int = 1):
        super(EmbeddingEncoder, self).__init__(vocab_count=vocab_count, latent_dim=latent_dim, pad_index=pad_index, dropout_ratio=dropout_ratio)
        ...
        self.rnn = nn.LSTM(
            input_size=self.embedding_dim, hidden_size=self.latent_dim, batch_first=True, bidirectional=False, num_layers=self.num_layers
        )
...
class OneHotEncoder(BaseEncoder):
    def __init__(self, vocab_count: int, latent_dim: int, pad_index: int, dropout_ratio: float =0.5, num_layers: int = 1):
        super(OneHotEncoder, self).__init__(vocab_count, latent_dim, pad_index, dropout_ratio)

        self.num_layers = num_layers
        self.rnn = nn.LSTM(input_size=self.vocab_count, hidden_size=self.latent_dim, batch_first=True, num_layers=self.num_layers)
